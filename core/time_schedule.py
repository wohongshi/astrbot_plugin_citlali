...
def set_config_path(path: str):
    ...
    _CUSTOM_CONFIG_PATH = os.path.join(path, "schedule_config.json")


def load_custom_schedule() -> dict:
    """加载用户自定义的日程配置"""
    if _CUSTOM_CONFIG_PATH and os.path.exists(_CUSTOM_CONFIG_PATH):
        try:
...
def save_custom_schedule(data: dict):
    """保存用户自定义的日程配置"""
    if _CUSTOM_CONFIG_PATH:
        os.makedirs(os.path.dirname(_CUSTOM_CONFIG_PATH), exist_ok=True)
...
def get_current_window() -> str:
    """获取当前时段 slug（使用配置的时区，默认 Asia/Shanghai）"""
    from datetime import timezone, timedelta

    # 读取时区设置
    tz_offset = 8  # 默认 UTC+8 (Asia/Shanghai)
    try:
        from .settings_manager import SettingsManager
        # 尝试读取，如果设置管理器还没初始化就用默认值
        import glob as _glob
        import json as _json
        if _CUSTOM_CONFIG_PATH:
            settings_path = os.path.join(os.path.dirname(_CUSTOM_CONFIG_PATH), "plugin_settings.json")
            if os.path.exists(settings_path):
                with open(settings_path, "r") as _f:
                    _s = _json.load(_f)
                    tz_offset = _s.get("timezone_offset", 8)
    except Exception:
        pass
    ...
    # 检查自定义时段
    custom = load_custom_schedule()
    custom_windows = custom.get("windows", {})
    if custom_windows:
        for slug, info in custom_windows.items():
...
def get_schedule_for_webui() -> dict:
    """获取完整的日程配置（用于 WebUI）"""
    custom = load_custom_schedule()
    result = {}
    for slug, name, default_start, default_end in SCHEDULE_WINDOWS:
        custom_w = custom.get("windows", {}).get(slug, {})
        schedule = CITLALI_SCHEDULE.get(slug, {})
        custom_s = custom.get("schedules", {}).get(slug, {})
        result[slug] = {
            "name": name,
            "start": custom_w.get("start", default_start),
            "end": custom_w.get("end", default_end),
            "activity": custom_s.get("activity", schedule.get("activity", "")),
            "mood": custom_s.get("mood", schedule.get("mood", "")),
            "location": custom_s.get("location", schedule.get("location", "")),
            "voice_style": custom_s.get("voice_style", schedule.get("voice_style", "")),
        }
    return result


def save_schedule_from_webui(data: dict):
    """从 WebUI 保存日程配置"""
    custom = load_custom_schedule()
    if "windows" not in custom:
        custom["windows"] = {}
    if "schedules" not in custom:
        custom["schedules"] = {}
    for slug, info in data.items():
        ...
        if any(k in info for k in ["activity", "mood", "location", "voice_style"]):
            custom["schedules"][slug] = {
                "activity": info.get("activity", ""),
                "mood": info.get("mood", ""),
                "location": info.get("location", ""),
                "voice_style": info.get("voice_style", ""),
            }
    save_custom_schedule(custom)


def get_time_context() -> str:
    """获取时段上下文"""
    window = get_current_window()
    schedule = get_effective_schedule(window)
    h = datetime.now().hour
...
def get_effective_schedule(slug: str) -> dict:
    """获取时段的有效配置（自定义覆盖默认）"""
    default = CITLALI_SCHEDULE.get(slug, {})
    custom = load_custom_schedule()
    custom_s = custom.get("schedules", {}).get(slug, {})
    if custom_s:
        merged = dict(default)
        merged.update({k: v for k, v in custom_s.items() if v})
        return merged
    return default
