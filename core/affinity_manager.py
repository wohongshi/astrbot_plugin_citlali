...
_custom_thresholds: list = []


def set_stage_thresholds(thresholds: list):
    """动态设置阶段阈值（从 WebUI 配置）"""
    global _custom_thresholds, STAGE_THRESHOLDS
    if len(thresholds) == 6:
        _custom_thresholds = thresholds
        stages = sorted(AffinityStage)
        for i, s in enumerate(stages):
            STAGE_THRESHOLDS[s] = thresholds[i]


def get_stage_thresholds() -> list:
    """获取当前阈值列表"""
    stages = sorted(AffinityStage)
    return [STAGE_THRESHOLDS[s] for s in stages]
